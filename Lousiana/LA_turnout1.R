...
# -------------------------
# Clean and aggregate to parish
# -------------------------
la_county_dat <- la_dat %>%
  mutate(
...
# Parish shapes
# -------------------------
la_counties <- counties(state = "LA", year = 2020, class = "sf") %>%
  mutate(county_fips = GEOID)

la_sf <- la_counties %>%
  left_join(la_county_dat, by = "county_fips")
...
la_plot <- ggplot(la_sf) +
  geom_sf(aes(fill = fill_group), color = "white", linewidth = 0.15) +
  scale_fill_manual(values = fill_colors, drop = FALSE) +
  labs(
    title = "Louisiana: Democratic vs Republican Turnout",
    subtitle = "Parish-level aggregation from 2024 L2 block turnout data",
...
ggsave("LA_PNG/louisiana_dem_vs_rep_county_map.png", la_plot, width = 9, height = 7, dpi = 300)