...
make_state_map <- function(state_code, state_name) {
  csv_path <- list.files(
    "Data_Folder/Cali_DATA/l2_stats",
    pattern = paste0("^", state_code, ".*\\.csv$"),
    full.names = TRUE,
    recursive = TRUE
  )[1]
  ...
  shp <- counties(state = state_code, year = 2020, class = "sf") %>%
    mutate(county_fips = GEOID) %>%
    left_join(county_dat, by = "county_fips")
  
  ggplot(shp) +
    geom_sf(aes(fill = fill_group), color = "white", linewidth = 0.15) +
    scale_fill_manual(values = fill_colors, drop = FALSE) +
    labs(
      title = paste0(state_name, ": Democratic vs Republican Turnout"),
      subtitle = "County/parish-level aggregation from 2024 L2 block turnout data",
...
ca_plot <- make_state_map("CA", "California")
la_plot <- make_state_map("LA", "Louisiana")

combined_plot <- ca_plot + la_plot + plot_layout(ncol = 2)

print(combined_plot)

ggsave("combined_CA_LA_turnout_map.png", combined_plot, width = 16, height = 8, dpi = 300)