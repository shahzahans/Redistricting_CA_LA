...
save(CA_vote_join, file = "CA_vote_join.RData")
...
# smaller sections of california starting with SOCAL
...
plan50_socal2 <- get_plans_matrix(plans_socal2)[, 50]
...
Socal2_plan50_map <- ggplot(socal2_plan50_vote) +
  geom_sf(aes(fill = margin_bin)) +
  scale_fill_manual(
    values = c(
      "R +15% or more"     = "#67001f",
...
midcal_map <- redist_map(
  ...
  pop_tol = 0.01,
  ndists = 17,   # or however many districts you want to draw
  total_pop = "pop",
  adj = adj_midcal
)
...
plan25_midcal <- get_plans_matrix(plans_midcal)[, 25]
...
midcal_plan25_map <- ggplot(midcal_plan25_vote) +
  geom_sf(aes(fill = margin_bin)) +
  scale_fill_manual(
    values = c(
...
plan100_norcal <- get_plans_matrix(plans_norcal)[, 100]
